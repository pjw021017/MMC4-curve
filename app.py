...
if "mmc4_pts" in st.session_state and "mmc4_C_hat" in st.session_state:
    ...
    # (1) 좌측(Shear 왼쪽): 직선 외삽 대신 포물선으로 "자연스럽게 상승"
    #     조건: eta=eta_min에서 값=e_min, eta=eta_min에서 기울기=0(최저점), 좌측으로 갈수록 증가
    e_min = float(mmc4_eps(eta_min, C_hat))
    delta = eta_min - eta_lo
    curv = abs(e_min) / (delta**2 + 1e-6) * 0.6  # 상승 강도(0.5~0.8 범위에서 튜닝 가능)
    ...
    # (2) 우측(Bulge 오른쪽): 기존처럼 끝점 기울기 유지(C1 연속)로 연결
    ...
    # ===== [추가 기능] 특정 x좌표(η_query) 입력 → y좌표(εf) 출력 =====
    ...
    # 곡선 정의(좌/내부/우)와 동일 규칙으로 단일 점 평가
